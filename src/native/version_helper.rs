...
//!
//! The `version_helper` module is an abstraction layer over the Version Helper API/Macro which
//! allow us to determine the Windows OS system version
//!
...
use windows::{
    Win32::{
        Foundation::{ERROR_OLD_WIN_VERSION, GetLastError},
        System::SystemInformation::{
            OSVERSIONINFOEXA, VER_MAJORVERSION, VER_MINORVERSION, VER_SERVICEPACKMAJOR,
            VerSetConditionMask, VerifyVersionInfoA,
        },
    },
    core::HRESULT,
};
...
// VER_GREATER_EQUAL == 3: the value fits in a u8
...
fn verify_system_version(major: u8, minor: u8, sp_major: u16) -> VersionHelperResult<bool> {
    let mut os_version = OsVersionInfo {
        // Fixed-size struct: the size cannot overflow a u32
        #[allow(clippy::cast_possible_truncation)]
        dwOSVersionInfoSize: size_of::<OsVersionInfo>() as u32,
